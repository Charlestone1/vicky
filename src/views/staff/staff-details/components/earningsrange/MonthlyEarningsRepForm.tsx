...
import { fetchMonthlyEarningsRepData } from '@/store/slices/earnings-report/earningsReportSlice'
...
const MonthlyEarningsRepForm = (props: CheckinreportProps) => {
    const { disableSubmit = false } = props
    const [submitting, setSubmitting] = useState(false)
    const { staffId } = useParams()
    const dispatch = useAppDispatch()

    const checkMonthlyEarningsReport = async (
        ...
        try {
            await dispatch(fetchMonthlyEarningsRepData({ staffId, monthName, year }))
        ...
        <div className='h-10'>
            <Formik
                ...
                onSubmit={(values, { setSubmitting }) => {
                    if (!disableSubmit) {
                        checkMonthlyEarningsReport(values)
                    } else {
                        setSubmitting(false)
                    }
                }}
...
export default MonthlyEarningsRepForm
...
type AddSingleTintRemovalFormSchema = {
    type: string
    name: string
    customerTime: number
    amount: number
...
const AddSingleRemoval = (props: AddSingleServicesFormProps) => {
    const { disableSubmit = false, className, setIsOpen } = props
    const { addSingleRemovalServices } = useAuth()
    ...
    const createSingleRemovalServices = async (
        values: AddSingleTintRemovalFormSchema,
        setSubmitting: (isSubmitting: boolean) => void
    ) => {
        const { type, name, amount, customerTime, timeOfCompletion } = values
        // console.log(values)
        setSubmitting(true)
        const result = await addSingleRemovalServices({ type, name, amount, timeOfCompletion, customerTime })
        ...
        <div className={className}>
            ...
            <Formik
                ...
                onSubmit={(values, { setSubmitting }) => {
                    if (!disableSubmit) {
                        createSingleRemovalServices(values, setSubmitting)
                    } else {
                        setSubmitting(false)
                    }
                }}
                                    ...
                                    >
                                        <Field name="timeOfCompletion">
                                            {({
                                                field,
                                                form,
                                            }: FieldProps<AddSingleTintRemovalFormSchema>) => (
                                                <Select
                                                    field={field}
                                                    form={form}
...
export default AddSingleRemoval

...
const AddFullTintInstall = (props: AddSingleServicesFormProps) => {
    const { disableSubmit = false, className, setIsOpen } = props
    const { addFullInstallServices } = useAuth()
    const [message, setMessage] = useTimeOutMessage()

    const dispatch = useAppDispatch()

    const createServices = async (
        values: AddFullTintFormSchema,
        setSubmitting: (isSubmitting: boolean) => void
    ) => {
        // const { type, name, defaultPrices, timeOfCompletion, filmQualityOrVehicleCategoryAmount } = values
        const { type, name, isFull, timeOfCompletion, customerTime, filmQualityOrVehicleCategoryAmount } = values
        // values.type = "installation"
        // console.log(values)

        setSubmitting(true)
        const result = await addFullInstallServices({ type, name, isFull, timeOfCompletion, customerTime, filmQualityOrVehicleCategoryAmount })
                                    ...
                                    >
                                        <Field name="timeOfCompletion">
                                            {({
                                                field,
                                                form,
                                            }: FieldProps<AddFullTintFormSchema>) => (
                                                <Select
                                                    field={field}
                                                    form={form}
...
export default AddFullTintInstall

...
import AddFullTintInstall from './AddFullTintInstall'
import EditFullTintInstall from './EditFullTintInstall'
    ...
    return (
        <div className="flex justify-between gap-x-2 my-4">
            ...
            <div>
                ...
                >
                    <h5 className="mb-4">Create Full Tint Installation Service</h5>
                    <AddFullTintInstall setIsOpen={setIsOpen} />
                </Dialog>
            </div>
        </div>
    )
...
const FullTintInstallTable = () => {
    const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([])
    const [globalFilter, setGlobalFilter] = useState('')
    const [dialogIsOpen, setIsOpen] = useState(false)
    const [updateDialogIsOpen, setUpdateDialogIsOpen] = useState(false)
    const [deleteDialogIsOpen, setDeleteDialogIsOpen] = useState(false)
    const [fullTint, setFullTint] = useState<Service[]>([])
        ...
        try {
            if (data) {
                const filteredService = data.filter(service =>
                    service.type === "installation" && service.isFull === true
                );

                setFullTint(filteredService);
            }
        } catch (error) {
            console.log(error as TypeError);
    ...
    //   For Pagination
    const totalData = fullTint?.length
    ...
    const columns = useMemo<ColumnDef<Service>[]>(
        () => [
            ...
            { header: 'Basic ASWT film', accessorKey: `filmQualityOrVehicleCategoryAmount.0.amount` },
            { header: 'Llumar ATC', accessorKey: `filmQualityOrVehicleCategoryAmount.1.amount` },
            { header: 'Llumar CTX (ceramic)', accessorKey: `filmQualityOrVehicleCategoryAmount.2.amount` },
            { header: 'Llumar IRX (nano-ceramic)', accessorKey: `filmQualityOrVehicleCategoryAmount.3.amount` },
            { header: 'Xpel prime XR plus', accessorKey: `filmQualityOrVehicleCategoryAmount.4.amount` },
            {
                        ...
                        onClick={() => {
                            setUpdateData({
                                ...
                                filmQualityOrVehicleCategoryAmount: [
                                    ...
                                    {
                                        filmQualityId: row.original.filmQualityOrVehicleCategoryAmount[2]?.filmQualityId.id,
                                        amount: row.original.filmQualityOrVehicleCategoryAmount[2]?.amount
                                    },
                                    {
                                        filmQualityId: row.original.filmQualityOrVehicleCategoryAmount[3]?.filmQualityId.id,
                                        amount: row.original.filmQualityOrVehicleCategoryAmount[3]?.amount
                                    },
                                    {
                                        filmQualityId: row.original.filmQualityOrVehicleCategoryAmount[4]?.filmQualityId.id,
                                        amount: row.original.filmQualityOrVehicleCategoryAmount[4]?.amount
                                    },
                                ]
    ...
    if (fullTint.length === 0) {
        return (
            <Card className="flex justify-center flex-col items-center min-h-[25rem]">
                <p className="text-center">No Full Tint Installation Services have been added</p>
        ...
        <div className="border border-gray-300 rounded-lg p-3">
            ...
            <div>
                <Table>
                    ...
                    >
                        <h5 className="mb-4">Update Full Tint Service Details</h5>
                    ...
                    >
                        <h5 className="mb-4">Delete Full Tint Installation Service</h5>
                        <DeleteService
                            disableSubmit
                            // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
                            data={oneDelete!}
...
export default FullTintInstallTable
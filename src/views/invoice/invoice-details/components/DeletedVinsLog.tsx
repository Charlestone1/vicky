...
function DebouncedInput({
    ...
    // const [activityLog, setActivityLog] = useState<EntryLogData[]>([])
    ...
    //     try {
    //         setLoading(true)
    //         const response = await axios.get(`${baseUrl}/entries/logs/${invoiceIdParams}`, {
    //             headers: {
...
const DeletedVinsLog = () => {
    const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([])
    const [globalFilter, setGlobalFilter] = useState('')
    const { invoiceIdParams } = useParams<{ invoiceIdParams: string }>()

    const [deleteLog, setDeleteLog] = useState<EntryLogData[]>([])
        ...
        try {
            if (data) {
                const filteredActivity = data.filter(entry =>
                    entry.document.description === "Vehicle Deleted"
                );
                setDeleteLog(filteredActivity);
            }
        } catch (error) {
            console.log(error as TypeError);
        }
    ...
    const totalData = deleteLog?.length
    ...
    if (error) {
        return (
            <div className="mt-6">
                <p>Error occured Loading Invoice Delete Log.</p>
            </div>
        )
    }
    ...
    if (deleteLog.length === 0) {
        return (
            <div className="flex items-center justify-center flex-col my-4">
                <p>No Vehicle has been deleted from this Invoice.</p>
            </div>
        )
    }
    return (
